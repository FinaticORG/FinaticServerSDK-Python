...
ORDERMODIFYQUERYPARAMSBASETIMEINFORCE_ONE_OF_SCHEMAS = ["DayTIF", "FOKTIF", "GTCTIF", "GTDTIF", "IOCTIF"]

class OrderModifyQueryParamsBaseTimeInForce(BaseModel):
    """OrderModifyQueryParamsBaseTimeInForce
    """

    # data type: DayTIF
    oneof_schema_1_validator: DayTIF | None = None
    # data type: GTCTIF
    oneof_schema_2_validator: GTCTIF | None = None
    # data type: GTDTIF
    oneof_schema_3_validator: GTDTIF | None = None
    # data type: IOCTIF
    oneof_schema_4_validator: IOCTIF | None = None
    # data type: FOKTIF
    oneof_schema_5_validator: FOKTIF | None = None
    actual_instance: DayTIF | FOKTIF | GTCTIF | GTDTIF | IOCTIF | None = None
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        instance = OrderModifyQueryParamsBaseTimeInForce.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str | None) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> dict[str, Any] | DayTIF | FOKTIF | GTCTIF | GTDTIF | IOCTIF | None:
