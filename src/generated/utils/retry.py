...
async def retry_api_call(
    ...
    status_list = retry_on_status or (
        config.retry_on_status if config else [429, 500, 502, 503, 504]
    )
    network_retry = (
        retry_on_network_error
        if retry_on_network_error is not None
        else (config.retry_on_network_error if config else True)
    )
    ...
    if network_retry:
        ...
        @retry(
            ...
            retry=retry_if_result(
                lambda r: False
            ),  # Never retry on result, only on exceptions
            reraise=True,
        )
        async def _retry_wrapper() -> T:
            try:
