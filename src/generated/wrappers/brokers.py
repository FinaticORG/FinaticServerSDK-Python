...
class BrokersWrapper:
    ...
    async def get_brokers(
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "GET", "/api/v1/brokers/", {}, self.sdk_config
            )
    ...
    async def connect_broker(
        self, broker_connection_request: BrokerConnectionRequest
    ) -> Any:
        """Connect Broker
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = await self.api.connect_broker_api_v1_brokers_connect_post(
                    broker_connection_request=broker_connection_request
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
    ...
    async def list_broker_connections(
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "GET", "/api/v1/brokers/connections", {}, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = (
                    await self.api.list_broker_connections_api_v1_brokers_connections_get()
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
    ...
    async def update_connection(
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "PUT",
                "/api/v1/brokers/connections/{connection_id}",
                {
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = await self.api.update_connection_api_v1_brokers_connections_connection_id_put(
                    connection_id=connection_id,
                    broker_connection_update_request=broker_connection_update_request,
                )
                # Apply response interceptors (Phase 2B)
    ...
    async def delete_connection(self, connection_id: str) -> Any:
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = await self.api.delete_connection_api_v1_brokers_connections_connection_id_delete(
                    connection_id=connection_id
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
    ...
    async def disconnect_broker(self, connection_id: str) -> Any:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "DELETE",
                "/api/v1/brokers/disconnect/{connection_id}",
                {"connection_id": connection_id},
                self.sdk_config,
            )
            cached = cache.get(cache_key)
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = await self.api.disconnect_broker_api_v1_brokers_disconnect_connection_id_delete(
                    connection_id=connection_id
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
    ...
    async def sandbox_callback(self, broker_id: str) -> Any:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "GET",
                "/api/v1/brokers/sandbox-callback/{broker_id}",
                {"broker_id": broker_id},
                self.sdk_config,
            )
            cached = cache.get(cache_key)
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = await self.api.sandbox_callback_api_v1_brokers_sandbox_callback_broker_id_get(
                    broker_id=broker_id
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
    ...
    async def oauth_callback_tastytrade(
        self,
    ) -> Any:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "GET", "/api/v1/brokers/callback/tastytrade", {}, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = (
                    await self.api.oauth_callback_tastytrade_api_v1_brokers_callback_tastytrade_get()
                )
                # Apply response interceptors (Phase 2B)
                return await apply_response_interceptors(response, self.sdk_config)
            ...
            # Structured logging (Phase 2B)
            self.logger.debug(
                "Oauth Callback Tastytrade completed",
                request_id=request_id,
                action="oauth_callback_tastytrade",
            )
            ...
            self.logger.error(
                "Oauth Callback Tastytrade failed",
                error=str(e),
                request_id=request_id,
                action="oauth_callback_tastytrade",
                exc_info=True,
    ...
    async def oauth_callback(self, broker_id: str) -> Any:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled:
            cache_key = generate_cache_key(
                "GET",
                "/api/v1/brokers/callback/{broker_id}",
                {"broker_id": broker_id},
                self.sdk_config,
            )
            cached = cache.get(cache_key)
        ...
        try:
            ...
            async def api_call():
                # Apply request interceptors (Phase 2B)
                response = (
                    await self.api.oauth_callback_api_v1_brokers_callback_broker_id_get(
                        broker_id=broker_id
                    )
                )
                # Apply response interceptors (Phase 2B)
