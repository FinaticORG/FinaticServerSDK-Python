...
class SessionWrapper:
    ...
    async def init_session(self, params: InitSessionParams) -> InitSessionResponse:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "POST", "/api/v1/session/init", params_dict, self.sdk_config
            )
    ...
    async def start_session(self, params: StartSessionParams) -> StartSessionResponse:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "POST", "/api/v1/session/start", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
    ...
    async def get_portal_url(self, params: GetPortalUrlParams) -> GetPortalUrlResponse:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "GET", "/api/v1/session/portal", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
    ...
    async def get_session_user(
        self, params: GetSessionUserParams
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "GET", "/api/v1/session/{session_id}/user", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
            ...
            async def api_call():
                response = (
                    await self.api.get_session_user_api_v1_session_session_id_user_get(
                        session_id=session_id, company_id=self.company_id
                    )
                )
    ...
    async def authenticate_session(
        self, params: AuthenticateSessionParams
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "POST", "/api/v1/session/authenticate", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
        ...
        try:
            ...
            async def api_call():
                response = (
                    await self.api.authenticate_session_api_v1_session_authenticate_post()
                )
    ...
    async def refresh_session(
        self, params: RefreshSessionParams
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "POST", "/api/v1/session/refresh", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
    ...
    async def test_webhook(self, params: TestWebhookParams) -> TestWebhookResponse:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "POST", "/api/v1/session/webhook/test", params_dict, self.sdk_config
            )
            cached = cache.get(cache_key)
            if cached:
                self.logger.debug(
        ...
        try:
            ...
            async def api_call():
                response = (
                    await self.api.test_webhook_api_v1_session_webhook_test_post()
                )
