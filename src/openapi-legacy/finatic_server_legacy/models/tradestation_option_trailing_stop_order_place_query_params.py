...
class TradestationOptionTrailingStopOrderPlaceQueryParams(BaseModel):
    ...
    option_route_kind: Optional[StrictStr] = Field(default=None, description="Hint for option route type when resolving via Get Routes (future): stock_option (STOCKOPTION) or index_option (INDEXOPTION). Ignored if ``route`` is set.", alias="optionRouteKind")
    order_qty: Annotated[int, Field(strict=True, gt=0)] = Field(alias="orderQty")
    order_type: StrictStr = Field(alias="orderType")
    route: Optional[StrictStr] = Field(default=None, description="Route name from Get Routes; optional. If set, used as-is; else \"Intelligent\".")
    ...
    __properties: ClassVar[List[str]] = ["accountId", "action", "assetType", "optionRouteKind", "orderQty", "orderType", "route", "stopPrice", "symbol", "timeInForce", "trailPercent", "trailPrice"]
