...
class AlpacaEquityTrailingStopOrderPlaceQueryParams(BaseModel):
    ...
    client_order_id: Optional[StrictStr] = Field(
        default=None, description="Client-provided order ID for tracking"
    )
    extended_hours: Optional[StrictBool] = Field(
        default=None, description="Allow execution during extended hours"
    )
    order_class: Optional[StrictStr] = Field(
        default=None, description="Order class: simple, bracket, oco, oto, mleg"
    )
    notional: Optional[Notional] = None
    take_profit: Optional[Dict[str, Any]] = Field(
        default=None, description="Take profit parameters for bracket/OCO orders"
    )
    stop_loss: Optional[Dict[str, Any]] = Field(
        default=None, description="Stop loss parameters for bracket/OCO/OTO orders"
    )
    legs: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Legs array for multi-leg orders"
    )
        ...
        _obj = cls.model_validate(
            {
                ...
                "assetType": (
                    obj.get("assetType")
                    if obj.get("assetType") is not None
                    else "equity"
                ),
                ...
                "orderQty": (
                    Orderqty.from_dict(obj["orderQty"])
                    if obj.get("orderQty") is not None
                    else None
                ),
                "stopPrice": obj.get("stopPrice"),
                "trail_percent": (
                    TrailPercent.from_dict(obj["trail_percent"])
                    if obj.get("trail_percent") is not None
                    else None
                ),
                "trail_price": (
                    TrailPrice.from_dict(obj["trail_price"])
                    if obj.get("trail_price") is not None
                    else None
                ),
                ...
                "notional": (
                    Notional.from_dict(obj["notional"])
                    if obj.get("notional") is not None
                    else None
                ),
