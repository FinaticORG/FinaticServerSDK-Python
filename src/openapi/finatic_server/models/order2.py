...
class Order2(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, Order2AnyOf1):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `Order2AnyOf1`"
            )
        else:
            return v
        ...
        if not isinstance(v, Order2AnyOf2):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `Order2AnyOf2`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in Order2 with anyOf schemas: Order2AnyOf, Order2AnyOf1, Order2AnyOf2. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Order2 with anyOf schemas: Order2AnyOf, Order2AnyOf1, Order2AnyOf2. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], Order2AnyOf, Order2AnyOf1, Order2AnyOf2]]:
