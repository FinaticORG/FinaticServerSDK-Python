...
class Order3(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if not isinstance(v, Order3OneOf1):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `Order3OneOf1`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in Order3 with oneOf schemas: Order3OneOf, Order3OneOf1. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in Order3 with oneOf schemas: Order3OneOf, Order3OneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into Order3 with oneOf schemas: Order3OneOf, Order3OneOf1. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Order3 with oneOf schemas: Order3OneOf, Order3OneOf1. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
