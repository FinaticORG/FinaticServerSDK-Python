...
class OrderModifyQueryParamsBase(BaseModel):
    ...
    order_id: Optional[StrictStr] = Field(
        default=None, description="Broker-assigned order identifier", alias="orderId"
    )
    ...
    time_in_force: Optional[OrderModifyQueryParamsBaseTimeInForce] = Field(
        default=None, alias="timeInForce"
    )
    is_automated: Optional[StrictBool] = Field(
        default=None,
        description="Indicates algorithmic/automated order source (optional)",
        alias="isAutomated",
    )
    ...
    order_qty: Optional[Annotated[int, Field(strict=True, gt=0)]] = Field(
        default=None, alias="orderQty"
    )
    ...
    def order_type_validate_enum(cls, value):
        ...
        if value not in set(["market", "limit", "stop", "trailing_stop"]):
            raise ValueError(
                "must be one of enum values ('market', 'limit', 'stop', 'trailing_stop')"
            )
        return value
    ...
    def asset_type_validate_enum(cls, value):
        ...
        if value not in set(
            ...
            raise ValueError(
                "must be one of enum values ('equity', 'equity_option', 'crypto', 'forex', 'future', 'future_option', 'bond')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "timeInForce": (
                    OrderModifyQueryParamsBaseTimeInForce.from_dict(obj["timeInForce"])
