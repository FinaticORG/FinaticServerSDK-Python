...
class OrderModifyQueryParamsBaseTimeInForce(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: Optional[str]) -> Self:
        ...
        if not _data_type:
            raise ValueError(
                "Failed to lookup data type from the field `timeInForce` in the input."
            )
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into OrderModifyQueryParamsBaseTimeInForce with oneOf schemas: DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
    ...
    def to_dict(
        self,
    ) -> Optional[Union[Dict[str, Any], DayTIF, FOKTIF, GTCTIF, GTDTIF, IOCTIF]]:
