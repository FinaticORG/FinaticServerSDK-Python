...
class OrderRequest(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if not isinstance(v, Trading212OrderModifyRequest):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `Trading212OrderModifyRequest`"
            )
        else:
            return v
        ...
        if not isinstance(v, WebullOrderModifyRequest):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `WebullOrderModifyRequest`"
            )
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in OrderRequest with anyOf schemas: NinjaTraderOrderModifyRequest, RobinhoodOrderModifyRequest, TastyTradeOrderModifyRequest, Trading212OrderModifyRequest, WebullOrderModifyRequest. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into OrderRequest with anyOf schemas: NinjaTraderOrderModifyRequest, RobinhoodOrderModifyRequest, TastyTradeOrderModifyRequest, Trading212OrderModifyRequest, WebullOrderModifyRequest. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
