...
class RobinhoodEquityTrailingStopOrderPlaceQueryParams(BaseModel):
    ...
    symbol: StrictStr
    order_qty: Orderqty1 = Field(alias="orderQty")
    ...
    extended_hours: Optional[StrictBool] = Field(
        default=False,
        description="Allow trading during extended hours (premium users only)",
        alias="extendedHours",
    )
    market_hours: Optional[StrictStr] = Field(
        default="regular_hours",
        description="Market hours to trade in",
        alias="marketHours",
    )
    ...
    def market_hours_validate_enum(cls, value):
        ...
        if value not in set(["regular_hours", "all_day_hours", "extended_hours"]):
            raise ValueError(
                "must be one of enum values ('regular_hours', 'all_day_hours', 'extended_hours')"
            )
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "orderQty": (
                    Orderqty1.from_dict(obj["orderQty"])
                ...
                "extendedHours": (
                    obj.get("extendedHours")
                    if obj.get("extendedHours") is not None
                    else False
                ),
                "marketHours": (
                    obj.get("marketHours")
                    if obj.get("marketHours") is not None
                    else "regular_hours"
                ),
            }
        )
