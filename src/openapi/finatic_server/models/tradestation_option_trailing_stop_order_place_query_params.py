...
class TradestationOptionTrailingStopOrderPlaceQueryParams(BaseModel):
    """
    ...
    account_id: Optional[Annotated[str, Field(strict=True, max_length=64)]] = Field(
        default=None, alias="accountId"
    )
    route: Optional[StrictStr] = Field(
        default=None,
        description='Route name from Get Routes; optional. If set, used as-is; else "Intelligent".',
    )
    option_route_kind: Optional[StrictStr] = Field(
        default=None,
        description="Hint for option route type when resolving via Get Routes (future): stock_option (STOCKOPTION) or index_option (INDEXOPTION). Ignored if ``route`` is set.",
        alias="optionRouteKind",
    )
    ...
    def option_route_kind_validate_enum(cls, value):
        ...
        if value not in set(["stock_option", "index_option"]):
            raise ValueError(
                "must be one of enum values ('stock_option', 'index_option')"
            )
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TradestationOptionTrailingStopOrderPlaceQueryParams from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # and model_fields_set contains the field
        if (
            self.option_route_kind is None
            and "option_route_kind" in self.model_fields_set
        ):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TradestationOptionTrailingStopOrderPlaceQueryParams from a dict"""
        if obj is None:
            return None
