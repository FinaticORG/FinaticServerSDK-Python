...
class UserBrokerConnectionWithPermissions(BaseModel):
    ...
    permissions: Optional[Dict[str, StrictBool]] = Field(
        default=None,
        description="Permissions for the current company (read/write access)",
    )
    requires_customer_agent: Optional[StrictBool] = Field(
        default=None,
        description="True when connection uses a customer-hosted push agent (MT4/MT5).",
    )
    push_agent_connector_state: Optional[StrictStr] = Field(
        default=None,
        description="MT connector lifecycle state when requires_customer_agent is true.",
    )
    push_agent_last_heartbeat_at: Optional[datetime] = Field(
        default=None, description="Last EA heartbeat timestamp from mt_connectors."
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * Fields in `self.additional_properties` are added to the output dict.
        ...
        # and model_fields_set contains the field
        if (
            self.requires_customer_agent is None
            and "requires_customer_agent" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.push_agent_connector_state is None
            and "push_agent_connector_state" in self.model_fields_set
        ):
        ...
        # and model_fields_set contains the field
        if (
            self.push_agent_last_heartbeat_at is None
            and "push_agent_last_heartbeat_at" in self.model_fields_set
        ):
