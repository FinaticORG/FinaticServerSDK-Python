"""Hand-maintained company wrapper over the OpenAPI-generated client."""
    ...
    async def get_company(self, **kwargs) -> FinaticResponse[Accounts]:
        ...
        if cache and self.sdk_config and self.sdk_config.cache_enabled and should_cache:
            ...
            cache_key = generate_cache_key(
                "GET", "/api/beta/company/{company_id}", params_dict, self.sdk_config
            )
        ...
        try:
            ...
            async def api_call():
                response = await self.api.get_company_api_beta_company_company_id_get(
            ...
            self.logger.debug(
                "Get Company completed", request_id=request_id, action="get_company"
            )
